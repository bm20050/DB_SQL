...
-- select 수강신청번호, 학번, 날짜 
-- from 수강신청 
-- where 수강신청.학번 not in (select 학번
-- 							from 학생
-- 							where 학생.학과 in (select 학과번호
-- 											    from 학과
-- 											    where 학과명 = '컴퓨터정보학과');
...
select distinct 학생.학번, 학생.이름, 학과, if(학과.학과명 = '컴퓨터정보학과', 학과명, '타과') as 비고
from 학생, 학과
where 학생.학과 = 학과.학과번호; 
...
from 수강신청내역, 과목
where 수강신청내역.과목번호 = 과목.과목번호 and 수강신청번호 in ('1610001', '1810001');
...
where 수강신청.수강신청번호 = 수강신청내역.수강신청번호 and 수강신청내역.과목번호 = 과목.과목번호 and 수강신청.학번 = 학생.학번 and 학생.학번 = '1801001';
...
where 과목.과목번호 = 수강신청내역.과목번호 and 수강신청내역.수강신청번호 = 수강신청.수강신청번호 and 수강신청.학번 = 학생.학번 and 학생.학번 = '1801001';

-- 69 수강신청 별로 신청 학생의 학번, 이름, 평균 평점(평점합계/과목수)를 출력하는 SQL 문을 작성하시오. (단, 학점이 부여된 수강신청만 출력한다) 
select 수강신청.수강신청번호, 학생.학번, 이름, avg(평점) '평균평점'
from 수강신청, 수강신청내역, 학생
where 수강신청.수강신청번호 = 수강신청내역.수강신청번호 and 수강신청.학번 = 학생.학번 and 평점 <> -1
...
select 학생.학번, 이름, count(*) '과목수', avg(평점) '평균평점'
from 학생, 수강신청, 수강신청내역
where 학생.학번 = 수강신청.학번 and 수강신청.수강신청번호 = 수강신청내역.수강신청번호 and 평점 <> -1
group by 학생.학번;

-- 76 과목별로 수강한 인원, 부여된 평점의 평균을 출력하시오. (단, 1 학기 과목에 한한다) 
select 과목.과목번호, 과목명, count(*) '수강인원', avg(평점) '평균평점'
from 과목, 수강신청내역
where 과목.과목번호 = 수강신청내역.과목번호 and 평점 <> -1 and 과목.학기 = '1'
group by 과목번호;